...
function FormDetailsTable({
  ...
  const renderTableColumns = (tableHead, dataRow, applicationIndex) => {
    if (tableHead === "name") {
      return (
        <RegisteredName
          localStorageKey={filterOptionsKey}
          dataRow={dataRow}
      ...
      return (
        <Box
          onClick={() => {
            if (!isActionDisable) {
              setClickedApplicationId([dataRow.application_id]);
  ...
  const handleAllCheckbox = (e) => {
    if (e.target.checked === true) {
      ...
      const adminSelectedApplications = JSON.parse(
        localStorage.getItem(`${Cookies.get("userId")}${selectedFormKey}`)
      );
    ...
    <Box className="basicDetailsTable">
            ...
            >
              {searchedEmail?.length === 0 && (
                <Pagination
                  ...
                  onPageChange={(page) =>
                    handleChangePage(page, savedPageNumberKey, setPage)
                  }
                  count={count}
                />
              )}
              ...
              {searchedEmail?.length === 0 && (
                <AutoCompletePagination
                  ...
                  page={page}
                  setPage={setPage}
                  localStorageChangeRowPerPage={savedRowPerPageKey}
                  localStorageChangePage={savedPageNumberKey}
      ...
      >
        <Box className="rearrange-columns-dialog-head">
          <Typography className="rearrange-columns-dialog-title">
...
export default React.memo(FormDetailsTable);
