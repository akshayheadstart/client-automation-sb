...
const CreateManagerAccount = () => {
  ...
  const [selectedSuperAccountManager, setSelectedSuperAccountManager] =
    useState(null);
  ...
  const [superAccountManagerList, setAccountManagerList] = useState([]);
  const [createAccountList] = useCreateManagerAccountMutation();
  //Create Manager account Head Title add
  useEffect(() => {
    setHeadTitle("");
    document.title = "Create Manager Account";
  }, [headTitle]);
  //Super Account Manager List API implementation here
  const { data, isSuccess, isError, error } =
    useGetSuperAccountManagerListQuery({});
  useEffect(() => {
    if (isSuccess) {
      const userList = [];
      data?.super_account_managers?.forEach((role) => {
        userList.push({
          label: `${role?.first_name} ${role?.middle_name} ${role?.last_name}`,
          value: role._id,
        });
      });
      setAccountManagerList(userList);
    } else if (isError) {
  ...
  const formik = useFormik({
    initialValues: {
      ...
      phone: "",
      email: "",
      associated_super_account_manager: "",
    },
    onSubmit: (values) => {
      ...
      const payload = {
        ...
        associated_super_account_manager: selectedSuperAccountManager?.value,
      };
        ...
        .then((response) => {
          try {
            if (response.message) {
              ...
              setEmail("");
              setMobileNumber("");
              setSelectedSuperAccountManager(null);
            } else {
                ...
                createAccountInternalServerError ? (
                  ...
                  <>
                    ...
                    >
                      ...
                      />
                      <Autocomplete
                        value={selectedSuperAccountManager}
                        getOptionLabel={(option) => option?.label}
                        onChange={(_, value) => {
                          setSelectedSuperAccountManager(value);
                        }}
                        fullWidth
                        options={superAccountManagerList}
                        renderInput={(params) => (
                          <TextField
                            color="info"
                            {...params}
                            label="Select Super Account Manager"
                            size="small"
                            required
                          />
                        )}
