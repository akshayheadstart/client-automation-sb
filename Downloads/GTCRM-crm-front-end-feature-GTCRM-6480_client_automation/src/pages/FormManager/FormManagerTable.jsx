...
import ExtraFilterList from "../ApplicationManager/ExtraFilterList";
import FormDetailsTable from "../../components/ui/application-manager/FormDetailsTable";
import { localStorageKeyNameOfFormManager } from "../../constants/LocalStorageKeyNames";
...
function FormManagerTable() {
  ...
  const {
    ...
    addedColumnKey,
    selectedFormKey,
  } = localStorageKeyNameOfFormManager;
  useEffect(() => {
  ...
  const tableRowPerPage = localStorage.getItem(
    `${Cookies.get("userId")}${savedRowPerPageKey}`
  )
    ? parseInt(
        localStorage.getItem(`${Cookies.get("userId")}${savedRowPerPageKey}`)
      )
    : 25;
  ...
  const [addedColumnsOrder, setaddedColumnsOrder] = useState(() => {
    if (
      JSON.parse(
        localStorage.getItem(`${Cookies.get("userId")}${addedColumnKey}`)
      ) === null
    ) {
      localStorage.setItem(
        `${Cookies.get("userId")}${addedColumnKey}`,
        JSON.stringify([])
      );
      return [];
    }
  ...
  const advFilterLocalStorageKey = `${Cookies.get(
    "userId"
  )}formManagerAdvanceFilterOptions`;
  ...
  const handleFilterOption = useCallback(
    (value) => {
      handleDataFilterOption(value, filterOptionsKey);
    },
    [filterOptionsKey]
  );

  useEffect(() => {
    if (state?.todays_assigned) {
      setShowLeadStageCount(true);
    }
  }, [state]);
  ...
  useEffect(() => {
    if (callFilterSaveApi) {
      ...
      const params = {
        ...
        }/admin/filter/?forms_filter=true&${
  ...
  const counselorListApiCallInfo = useGetCounselorListQuery(
    ...
    {
      skip: bothAreTrue(
        callFilterOptionApi.skipCounselorApiCall,
  ...
  //get counsellor list
  useEffect(() => {
    if (!callFilterOptionApi.skipCounselorApiCall || !skipCounselorApiCall) {
  ...
  const [items, setItems] = useState(() => {
    if (
      JSON.parse(
        localStorage.getItem(`${Cookies.get("userId")}${arrangedColumnKey}`)
      ) === null
    ) {
      localStorage.setItem(
        `${Cookies.get("userId")}${arrangedColumnKey}`,
  ...
  const handleCustomizeTableColumn = useCallback(() => {
    const filterOptions = JSON.parse(
      localStorage.getItem(`${Cookies.get("userId")}${filterOptionsKey}`)
    );
  ...
  useEffect(() => {
    ...
    if (state?.admin_dashboard) {
      ...
      const filterOptions = localStorage.getItem(
        `${Cookies.get("userId")}${filterOptionsKey}`
      );
      if (filterOptions) {
  ...
  useEffect(() => {
    apiCallFrontAndBackPage(
      ...
      { payload: allApplicationPayload, formInitiated: true },
      twelveScoreSort
    );
    ...
    (downloadButtonName, application) => {
      ...
      if (
        ...
        customFetch(
          `${
            import.meta.env.VITE_API_BASE_URL
          }/admin/download_applications_data/?form_initiated=false&${
            currentUserCollegeId ? "college_id=" + currentUserCollegeId : ""
          }`,
                ...
                try {
                  if (typeof expectedData === "string") {
                    ...
                    localStorage.removeItem(
                      `${Cookies.get("userId")}${selectedFormKey}`
                    );
                  } else {
                    throw new Error(
  ...
  const handleSearchByEmail = () => {
    ...
    customFetch(
      `${
        import.meta.env.VITE_API_BASE_URL
      }/admin/all_applications_by_email/?search_input=${searchedEmail}${
        currentUserCollegeId ? "&college_id=" + currentUserCollegeId : ""
      }`,
  ...
  const handleUpdatePageNumber = () => {
    setPageNumber(1);
    localStorage.setItem(`${Cookies.get("userId")}${savedPageNumberKey}`, 1);
  };

  const resetAllTheSelectedFilters = () => {
    localStorage.removeItem(`${Cookies.get("userId")}${filterOptionsKey}`);
            ...
            <>
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  localStorage.removeItem(
                    `${Cookies.get("userId")}selectedForms`
                  );
                  setSelectedApplications([]);
                  setSelectedEmails([]);
  ...
  const handleSaveFilters = (event) => {
    ...
    const params = {
      apiURL: `${
        import.meta.env.VITE_API_BASE_URL
      }/admin/filter/add/?forms_filter=true&${
        currentUserCollegeId ? "college_id=" + currentUserCollegeId : ""
      }`,
      payload,
  ...
  const handleDeleteSaveFilter = () => {
    ...
    const params = {
      apiURL: `${
        import.meta.env.VITE_API_BASE_URL
      }/admin/filter/delete_by_name/?forms_filter=true&filter_name=${deleteFilterName}${
        currentUserCollegeId ? "&college_id=" + currentUserCollegeId : ""
      }`,
      setCallFilterSaveApi,
      setListOfFilters,
  ...
  const handleResetAllTheFilters = useCallback(() => {
    resetFilterOptions();
    setFilterPayload("");
    localStorage.removeItem(`${Cookies.get("userId")}selectedForms`);
    setSelectedApplications([]);
    setSelectedEmails([]);
    setSelectedMobileNumbers([]);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    setHeadTitle("Form Manager");
    document.title = "Form Manager";
  }, [headTitle]);
  return (
    <>
          ...
          <Container maxWidth={false}>
              ...
              <Grid item md={12} sm={12} xs={12}>
                ...
                {(showLeadStageCount ||
                  ...
                  <LeadStageCountAndDetails
                    ...
                    openTodayAssignedByDefault={state?.todays_assigned}
                  />
                )}
                <Card
                    ...
                    >
                      ...
                      >
                        <TableTopPagination
                          ...
                          localStoragePageNumberKey={savedPageNumberKey}
                          rowsPerPage={rowsPerPage}
                          totalCount={totalApplicationCount}
                        />
                        {showGenerateRequest && (
            ...
            {selectedApplications?.length > 0 && !isActionDisable && (
              <LeadActions
                ...
                localStorageKey={selectedFormKey}
