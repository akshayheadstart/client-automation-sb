    ...
    const t = timer((elapsed) => {
      ...
      const currNodes = transitions.map(({ node, from, to }) => {
        return {
          ...
          data: { ...node.data },
          type: node.type,
          delay_data: node?.delay_data ? node?.delay_data : null,
          communication_data: node?.communication_data
            ? node?.communication_data
            : null,
          if_else_data: node?.if_else_data ? node?.if_else_data : null,
          exit_condition_data: node?.exit_condition_data
            ? node?.exit_condition_data
            : null,
          tag_data: node?.tag_data ? node?.tag_data : null,
          lead_stage_data: node?.lead_stage_data ? node?.lead_stage_data : null,
          allocation_counsellor_data: node?.allocation_counsellor_data
      ...
      if (elapsed > options.duration) {
        ...
        const finalNodes = transitions.map(({ node, to }) => {
          return {
            id: node.id,
            position: {
              x: to.x - (node.width ?? 0) / 2,
              y: to.y - (node.height ?? 0) / 2,
            },
            data: { ...node.data },
            type: node.type,
            delay_data: node?.delay_data ? node?.delay_data : null,
            ...
            exit_condition_data: node?.exit_condition_data
              ...
              : null,
            tag_data: node?.tag_data ? node?.tag_data : null,
            lead_stage_data: node?.lead_stage_data
              ? node?.lead_stage_data
              : null,
            allocation_counsellor_data: node?.allocation_counsellor_data
              ? node?.allocation_counsellor_data
              : null,
            if_else_condition: node?.if_else_condition
